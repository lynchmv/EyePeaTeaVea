...
# --- Start of New, More Comprehensive Genre Mapping Logic ---
# This dictionary is now based on the detailed mappings from Mediafusion.
GENRE_MAP = {
    "Football": ["soccer", "football"],
    "Cricket": ["cricket"],
    "Tennis": ["tennis"],
    "Motor Sport": ["motorsport", "race", "f1"],
    "Boxing": ["boxing"],
    "MMA": ["mma", "ufc"],
    "Golf": ["golf"],
    "American Football": ["am. football", "american football", "nfl", "gridiron"],
    "Athletics": ["athletics"],
    ...
    "GAA": ["gaa"],
    "Fighting": ["fight", "wrestling"],
    "Other Sports": ["snooker", "clubber"],
}

def map_genre(group_title: str) -> str:
    """Maps a specific group title to a generic sport genre."""
    group_lower = group_title.lower()
    for generic_genre, keywords in GENRE_MAP.items():
        for keyword in keywords:
            if keyword in group_lower:
                return generic_genre
    return "Other Sports" # Default category if no keyword matches
# --- End of New Genre Mapping Logic ---


class CombinedPlaylistParser:
    """
    Fetches multiple source playlists, combines them, and parses the result,
    correctly separating channels and events.
    """
    ...
    async def parse(self) -> dict:
        ...
        for channel in channels:
            if processed_count > 0 and processed_count % 65 == 0:
            ...
            name_to_check = channel.attributes.get(IPTVAttr.TVG_NAME.value, channel.name).strip()
            is_event = date_pattern.search(name_to_check) and time_pattern.search(name_to_check)

            if is_event:
                try:
                    found_dates = search_dates(name_to_check)
                    if not found_dates:
                    ...
                    unique_str = f"{name_to_check}{event_start_timestamp}"
                    ...
                    # Use the mapping function to get a clean genre
                    event_genre = map_genre(group_title)
                    ...
                    event_metadata = models.MediaFusionEventsMetaData(
                        ...
                        genres=[event_genre], # Assign the mapped genre
                        streams=[models.TVStreams(meta_id=event_id, name=name_to_check, url=channel.url, source=self.playlist_source)]
                    )
                    parsed_data["events"].append(event_metadata)
                except Exception as e:
                    logging.warning(f"Skipping event due to processing error: '{name_to_check}' | Error: {e}")
            else:
                clean_name = channel.attributes.get(IPTVAttr.TVG_NAME.value, channel.name)
                ...
                poster_url = channel.attributes.get(IPTVAttr.TVG_LOGO.value)
                group_title = channel.attributes.get('group-title', 'Uncategorized')
